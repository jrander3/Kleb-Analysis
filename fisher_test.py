...
for gene in df.columns[:-1]:  # last column is 'source'
    ...
    if group_counts.shape != (2, 2):
        continue  # skip genes that are only present in one group
...
df_results.to_csv("arg_matrix/fisher_enrichment_results.csv", index=False)

print("✅ Fisher's test results saved to arg_matrix/fisher_enrichment_results.csv")
